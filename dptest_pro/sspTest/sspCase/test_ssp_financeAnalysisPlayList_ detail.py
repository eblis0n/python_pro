...
class test_queryentry_ssp_analysisPlayList_detail (unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        print("queryentry_ssp_analysisPlayList_detail(数据明细)接口测试开始\n")
    ...
    # 数据明细统计
    def test_analysisPlayDetailLen(self):
        print("开始测试test_analysisPlayDetailLen")
        # 执行接口查询
        ...
        self.assertEqual(len(inf_analysisplaylistlen), len(sql_dataLen),"test_analysisPlayDetailLen（数据明细）,数据对不上" )

    # 数据明细_有效屏
    @skip_dependon(depend="test_analysisPlayDetailLen")
